...
function run (workingDirPath, config, runMethodCallback) {
    ...
    if ( ! (workingDirPath && config && config.indexedSpatialDataFileName) ) { 
        throw new Error('Need to pass the working directory and a config object with the indexedSpatialDataFileName property defined.');
    }
    ...
    function fitStopsToPath (stop_ids, stopPointCoords, waypointCoords, tripID, shapeID) {
        ...
        var stopPoints      = getGeoJSPointsForGTFSPoints(stopPointCoords),
            ...
            metadata = {
                __originStopID : stopProjections[0].stop_id ,
                __shapeID      : shapeID                    ,
            };
        ...
        return (Array.isArray(stopProjections) && stopProjections.length) ? 
                    ...
                    stopProjections.reduce(function (acc, projection, i) {
...
// 
//fs.writeFile('/home/paul/AVAIL/GTFSrt_to_SIRI/MTA_Subway_SIRI_Server/data/GTFS/keys.txt', log2);


//FIXME: This is MTA Subway specific code that compensates for holes in the GTFS data.
//       This belongs in the GTFS config.
//
// Goes after the _.forEach(results.tripID2ShapeID, function (shapeID, tripID) 
//      in  `buildTheSpatialDataDictionary`
//
//(function () {
    //var patcher = Object.keys(stopsAndPathsToMemoTableIndex).reduce(function (acc, key, index) {
        //var keySplit = key.split('|'),
            //route = keySplit[0].substring(0, keySplit[0].indexOf('.'));
            ////shapeID = projectionsMemoTable[stopsAndPathsToMemoTableIndex[key]].__shapeID;
    ...
    //patcher = _.forEach(patcher, function (coveredStopsArrays) {
        //coveredStopsArrays.sort(function (a,b) { 
    ...
    //_.forEach(shapelessTrips, function (trip_id) {
        //if (trip_id.charAt(0) !== 'A') { return; }

        //var shape = trip_id.split('_')[2],
            //route = shape.substring(0, shape.indexOf('.')),
            ////route = shape.substring(shape.lastIndexOf('.')+1),
            //stops = results.tripID2StopIDs[trip_id],
            //stopArrs = patcher[route],
            //i, ii, iii;
         
    //console.log(route);
        //if (!stopArrs) { return; }

        //for ( i = 0 ; i < stopArrs.length ; ++i) { 
            //iii = 0;
            //for ( ii = 0; ii < stops.length; ++ii) {
                //for ( ; iii < stopArrs[i].stops.length; ++iii ) {
                    //if ( stops[ii] === stopArrs[i].stops[iii] ) {
                        //break; 
                    //}
                //}
                //if ( iii === stopArrs[i].stops.length ) { break; }
            //}
            //if ( ii === stops.length ) { break; }
        //}

        //if ( i < stopArrs.length ) {
            //console.log('Found a match for', trip_id);
            //tripKeyToMemoTableIndex[config.tripKeyBuilder(trip_id)] = stopArrs[i].index;
        //} else {
            //console.log('No match for', trip_id);
            //log += '\n==========================================\n';
            //log += 'No match for ' + trip_id + '\n';
            //log += 'stops:\n' + JSON.stringify(stops) + '\n';
            //log += '\n' + JSON.stringify(stopArrs, null, '    ') + '\n';
            //log += '\n==========================================\n';
        //}
    //});

    //fs.writeFile('/home/paul/AVAIL/GTFSrt_to_SIRI/MTA_Subway_SIRI_Server/data/GTFS/log.txt', log);
//}());
