...
function run (workingDirPath, config, runMethodCallback) {
    ...
    if ( ! (workingDirPath && config && config.indexedSpatialDataFilePath) ) { 
        throw new Error('Need to pass the working directory and a config object with the indexedSpatialDataFilePath property defined.');
    }
    ...
    function buildTheSpatialDataDictionary (err, results) {
        ...
        async.parallel([ outputTheIndexedSpatialData.bind(null, theIndexedSpatialData), 
                         outputTheIndexingStatistics.bind(null, statsHeader) ], 
    ...
    function outputTheIndexedSpatialData (theIndexedSpatialData, callback) {
        var writeStream = fs.createWriteStream(config.indexedSpatialDataFilePath);

        writeStream.write(JSON.stringify(theIndexedSpatialData));
        writeStream.on('end', callback);
        writeStream.on('error', callback); // Pass the error along to the callback.
    }


    function outputTheIndexingStatistics (statsHeader, callback) {
        if (LOG_STATS) {

            var writeStream = fs.createWriteStream(config.indexingStatisticsFilePath);

            writeStream.write(JSON.stringify(indexingStatistics));
            writeStream.on('end', callback);
            writeStream.on('error', callback); // Pass the error along to the callback.
