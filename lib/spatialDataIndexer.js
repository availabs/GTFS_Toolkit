...
function run (workingDirPath, config, runMethodCallback) {
    ...
    var _tripKeyMutator = (Array.isArray(config.tripKeyMutator))
            ? [new RegExp(config.tripKeyMutator[0]) , (config.tripKeyMutator[1] || '')]
            : null ;
    ...
    function buildTheSpatialDataDictionary (err, results) {
        ...
        _.forEach(results.tripID2ShapeID, function (shapeID, tripID) { 
            ...
            var tripKey = (_tripKeyMutator)
                  ? tripID.replace(_tripKeyMutator[0], _tripKeyMutator[1]) 
                  : tripID,
            
                stop_ids       = results.tripID2StopIDs[tripKey],
            ...
            stopProjections = fitStopsToPath(stop_ids,stopPointCoords,waypointCoords,tripKey,shapeID);

            tripKeyToMemoTableIndex[tripKey] = 
                stopsAndPathsToMemoTableIndex[stopsToPathKey] =
                  (stopProjections && projectionsMemoTable.length);

            if (stopProjections) {
                projectionsMemoTable.push(stopProjections);
            } else {
                indexingStatistics.tripsWithoutProjections.push(tripKey);
            }
        });
    ...
    function fitStopsToPath (stop_ids, stopPointCoords, waypointCoords, tripKey, shapeID) {
        ...
        var stopPoints   = getGeoJSPointsForGTFSPoints(stopPointCoords),
            ...
            if ( ! stopProjections ) {
                indexingStatistics.tripsRequiringRegression.push(tripKey);
                stopProjections = fitStopsToPathUsingLeastSquares(theTable);
            }

            if (LOG_STATS) {
                logStatsForStopsToPathProjection(stopProjections, tripKey);
            }
    ...
    function logStatsForStopsToPathProjection (stopProjections, tripKey) {
        var projection ,
            deviationInFt ,
            i ;
        ...
        for (i=0; i < stopProjections.length; ++i) {
            ...
            if (deviationInFt > DEVIANCE_THRESHOLD) {
                if (!indexingStatistics.tripsWithAnomalies[tripKey]) {
                    indexingStatistics.tripsWithAnomalies[tripKey] = {};
                }

                indexingStatistics.tripsWithAnomalies[tripKey][projection.stop_id] = {
    ...
    function getTripID2StopIDs (cbak) {
        ...
        converter.on("end_parsed", function (parsedTable) {
            ...
            _.forEach(parsedTable, function (row) {
              var tripKey = (_tripKeyMutator)
                ? row.trip_id.replace(_tripKeyMutator[0], _tripKeyMutator[1])
                : row.trip_id

              var _stopIDs = (stopTimesMap[tripKey] || (stopTimesMap[tripKey] = []));

              // We prefer to use stop_sequence, if it is provided.
              if (Number.isFinite(+row.stop_sequence)) {
                _stopIDs[+row.stop_sequence] = row.stop_id;
              } else {
                _stopIDs[+row.stop_sequence].push(row.stop_id)
              }
            });
      
            // Above we may have created a sparse array. Now we make it dense.
            _.forEach(stopTimesMap,
                (stopIDsArr, tripKey) => 
                  stopTimesMap[tripKey] = stopIDsArr.filter(s => (s || (s === 0)))
              )
