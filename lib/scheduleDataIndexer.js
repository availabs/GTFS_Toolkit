...
function run (workingDirPath, config, runMethodCallback) {
    ...
    var Converter   = require("csvtojson").Converter ,
        ...
        _tripKeyMutator = (Array.isArray(config.tripKeyMutator))
    ...
    (function iterateAndParse (i) {
        ...
        if ((tableName === 'stop_times') && (!config.indexStopTimes)) {
          return iterateAndParse(++i);
        }
        ...
        try {
            ...
            fs.createReadStream(filePath)
              .on('error', function (err) {
                    if (err.code === 'ENOENT') {
                        msg = 'No ' + filePath + ' in the data directory. Respective data excluded.' ;
            ...
            converter.on("end_parsed", function (parsedTable) {
                ...
                if (tableName === 'stop_times') {
                    ...
                    dataDict[tableName] = parsedTable.reduce(function (acc, rowObj) {
                        ...
                        var tripKey = (_tripKeyMutator)
                              ? rowObj.trip_id.replace(_tripKeyMutator[0], _tripKeyMutator[1])
                              : rowObj.trip_id ,
                        ...
                        if ((tripKey === previousTripKey) && (previousStopInfo)) {
                            previousStopInfo.nextStop = Object.assign({}, stopInfo)
                            previousStopInfo.nextStop.nextStop = undefined
                        }
                ...
                dataDict[tableName] = _.indexBy(parsedTable, function (rowObj) {
                    ...
                    if ((tableName === 'trips') && _tripKeyMutator) {
                        return rowObj[keyName].replace(_tripKeyMutator[0], _tripKeyMutator[1]);
                    } else {
