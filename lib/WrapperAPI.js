...
var theAPI = {
    ...
    getStopTimesBySequenceForStopForTrip : function (tripKey, stop_id, stop_sequence)  {

        var sequenceOfStops = _.get(this, ['indexedScheduleData', 'stop_times', tripKey, 'sequenceOfStops'], null) ,
            sequencesForStop ,
            stopTimes ;


        if ((sequenceOfStops === null) || (typeof sequenceOfStops !== 'object')) {
            return null;
        }
        ...
        if (isNaN(stop_sequence)) {
            sequencesForStop = 
                _.get(this, ['indexedScheduleData', 'stop_times', tripKey, 'stopIdToSequenceNumbers'], null);

            if (Array.isArray(sequencesForStop) && sequencesForStop.length) {
                stop_sequence = sequencesForStop[0];
            } else {
                stop_sequence = NaN;
            }
        }

        stopTimes = (!isNaN(stop_sequence) && sequenceOfStops[stop_sequence]);

        return (stopTimes || null);
    },


    getScheduledArrivalTimeForStopForTrip : function (tripKey, stop_id, stop_sequence)  {
        var stopTimesBySequence = this.getStopTimesBySequenceForStopForTrip(tripKey, stop_id, stop_sequence) ;
            
        return (stopTimesBySequence && stopTimesBySequence.arrival_time) || null;
    },
    ...
    getScheduledDepartureTimeForStopForTrip : function (tripKey, stop_id, stop_sequence)  {
        ...
        return (stopTimesBySequence && stopTimesBySequence.departure_time) || null;
    },
