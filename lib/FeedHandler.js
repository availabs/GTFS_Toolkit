...
function FeedHandler (config) {
    ...
    eventCreator.emitFeedHandlerConstructionEvent({
        debug: 'GTFS FeedHandler constructor called.' ,
    ...
    try {
        ...
        eventCreator.emitFeedHandlerConstructionEvent({
            debug: 'GTFS FeedHandler constructor completed without error.' ,
            timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) 
        }) ;
...
function getIndexedScheduleDataSync () {
    ...
    try {
        ...
        eventCreator.emitFeedHandlerConstructionEvent({
            debug: 'GTFS indexed schedule data successfully loaded from disk.' ,
            timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) 
        }) ;
...
function getIndexedSpatialDataSync () {
    ...
    try {
        ...
        eventCreator.emitFeedHandlerConstructionEvent({
            debug: 'GTFS indexed spatial data successfully loaded from disk.' ,
            timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) 
        }) ;
